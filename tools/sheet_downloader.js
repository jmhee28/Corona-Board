...
class SheetDownloader {
    ...
    async downloadToJson(spreadsheetId, sheetName, filePath = null){
        ...
        const object = this._rowsToObject(rows); // 내부함수 2차원 배열을 키값 형태로 변환

        //변환된 객체는 filePath가 지정된 경우 JSON 형태로 직렬화serialize해서 파일로 저장합니다.
    ...
    _rowsToObject(rows){
        ...
        return dataRows.map((row)=> { //3
            const item = {};
            
            for( let i = 0; i< headerRow.length; i++) // 열 수 만큼 반복
            {
